{
  description = "LuaJIT Development Flake: A hybrid package and module provider.";

  /*
    📘 NEW ENGINEER GUIDE: THE HYBRID FLAKE PATTERN
    ===============================================
    This file demonstrates a powerful Nix pattern that serves two purposes:
    1. It is a Package Provider: It builds a custom LuaJIT environment.
  ...
  outputs =
    ...
    flake-utils.lib.eachDefaultSystem (
      ...
      let
        ...
        # We construct a LuaJIT interpreter with specific libraries pre-loaded.
        luajitPackages = pkgs.luajit.withPackages (ps: [
          ps.cjson
          ps.luafilesystem
          ps.luasocket
          ps.penlight
        ...
        # We combine the interpreted LuaJIT environment with external tools (Language Server).
        devPackages = [
          luajitPackages
        ...
        luajitEnvPackage = pkgs.buildEnv {
          name = "luajit-dev-env";
          paths = devPackages;
        };
      ...
      in
      {
        ...
        packages.default = luajitEnvPackage;
    ...
    // {
      homeManagerModules.default =
        ...
        with lib;
        let
          cfg = config.programs.luajit;
          startupMessage = "LuaJIT development environment activated!";
        in
        {
          # 1. THE INTERFACE
          # We define an option that users can toggle in their home.nix
          # Usage: programs.luajit.enable = true;
          options.programs.luajit = {
            enable = mkEnableOption "LuaJIT development environment";
          };
          ...
          config = mkIf cfg.enable {
            ...
            home.activation.luajitInfo = lib.hm.dag.entryAfter [ "writeBoundary" ] ''
              echo "${startupMessage}"
            '';
          };
        };
