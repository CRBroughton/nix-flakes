{
  description = "Nim Development Flake: A hybrid package and module provider.";

  /*
    📘 NEW ENGINEER GUIDE: THE HYBRID FLAKE PATTERN
    ===============================================
    This file demonstrates a powerful Nix pattern that serves two purposes:
    1. It is a Package Provider: It builds a custom Nim environment.
  ...
  outputs =
    ...
    flake-utils.lib.eachDefaultSystem (
      ...
      in
      {
        ...
        packages.default = nimEnvPackage;
    ...
    // {
      homeManagerModules.default =
        ...
        with lib;
        let
          cfg = config.programs.nim;
          startupMessage = "Nim development environment activated!";
        in
        {
          # 1. THE INTERFACE
          # We define an option that users can toggle in their home.nix
          # Usage: programs.nim.enable = true;
          options.programs.nim = {
            enable = mkEnableOption "Nim development environment";
          };
          ...
          config = mkIf cfg.enable {
            ...
            home.activation.nimInfo = lib.hm.dag.entryAfter [ "writeBoundary" ] ''
              echo "${startupMessage}"
            '';
          };
        };
