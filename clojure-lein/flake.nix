{
  description = "Clojure Leiningen Development Flake: A hybrid package and module provider.";

  /*
    📘 NEW ENGINEER GUIDE: THE HYBRID FLAKE PATTERN
    ===============================================
    This file demonstrates a powerful Nix pattern that serves two purposes:
    1. It is a Package Provider: It builds a custom Clojure/Leiningen environment.
  ...
  outputs =
    ...
    flake-utils.lib.eachDefaultSystem (
      ...
      let
        ...
        # We construct a Clojure/Leiningen development environment.
        devPackages = [
          pkgs.clojure # The Clojure CLI tools (`clj`)
          pkgs.jdk25_headless # Java Development Kit 25 (headless)
          pkgs.leiningen # Project automation and dependency management
      ...
      in
      {
        ...
        devShell = pkgs.mkShell {
          ...
          shellHook = ''
            echo "=========================================="
            echo "Clojure + Leiningen Development Environment"
            echo "=========================================="
            echo ""
            echo "Included tools:"
            ...
            echo "  lein run"
            echo ""
            echo "Create a new Leiningen library:"
            echo "  lein new my-library"
            echo ""
    ...
    // {
      homeManagerModules.default =
        ...
        with lib;
        let
          cfg = config.programs.clojure-lein;
          startupMessage = "Clojure + Leiningen development environment activated!";
        in
        {
          # 1. THE INTERFACE
          # We define an option that users can toggle in their home.nix
          # Usage: programs.clojure-lein.enable = true;
          options.programs.clojure-lein = {
            enable = mkEnableOption "Clojure + Leiningen development environment";
          };
          ...
          config = mkIf cfg.enable {
            ...
            home.activation.clojureLeinInfo = lib.hm.dag.entryAfter [ "writeBoundary" ] ''
              echo "${startupMessage}"
            '';
          };
        };
